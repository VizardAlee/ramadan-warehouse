...
New transfers use **Request stock → Source manager approves → Receiving manager confirms arrival**. A system administrator may perform either action organization-wide. Approval holds the goods automatically; full good receipt updates both locations and completes the transfer. Direct branch-to-branch supply is supported without involving the main warehouse. Packaging, picking, dispatch and transport forms are not mandatory for this flow. See [Simple stock transfers](simple-stock-transfers.md) for permissions, exception handling, inventory semantics and release status.
