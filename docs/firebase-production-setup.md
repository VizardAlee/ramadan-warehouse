...
All additional production users are invite-only. Administrators must use **Administration → Users → Invite user**; there is no public sign-up path. The invited identity receives an unknown server-generated password and cannot sign in until the one-time password-setup link is completed. A Firebase Auth identity without the matching active organization profile and authorization-version claims receives no protected application access.
