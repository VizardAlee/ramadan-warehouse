...
If that one-time link expires before the invited user activates the account, an authorized user manager can select **Re-invite** in Administration → Users. The server preserves the existing roles and location assignments, confirms the Auth account has never signed in, issues a fresh one-time link, records the new expiry, and writes an audit event. Re-invitation is rejected for accounts that have already been used; those users must use the normal password-recovery flow instead.
