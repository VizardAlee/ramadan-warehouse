...
export default function AccountingClosePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setWorkspace(
        await callAdministration("getAccountingCloseWorkspace", { periodKey }),
      );
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : "The monthly close workspace could not be loaded.",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function run(action: () => Promise<unknown>, success: string) {
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "The accounting close action could not be completed.",
      );
    } finally {
      setBusy(false);
    }
  ...
  if (!profile || !can("accounting.close.read"))
    return (
      <div className="rounded-xl border bg-white p-6">
        Your roles do not include monthly accounting-close access.
      </div>
    );
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="max-w-3xl text-[var(--muted)]">
            Review the month&apos;s balanced ledger and operational blockers,
            prepare the evidence, then complete the close. Managers and
            administrators may complete their own preparation; every action is
            audited. A prepared or closed month rejects all new journal postings
            dated in that month.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-[16rem_minmax(0,1fr)] md:items-end">
          ...
          <div className="rounded-lg bg-slate-50 p-4 text-sm">
            <strong className="capitalize">
              {selectedPeriod?.status ?? "Open / not prepared"}
            </strong>
            <p className="mt-1 text-[var(--muted)]">
              {workspace?.evidence.periodStart} to{" "}
              {workspace?.evidence.periodEnd}
            </p>
          </div>
        </div>
      </section>
        ...
        <>
          <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-xl border bg-white p-4">
              ...
              <strong className="mt-2 block text-2xl">
                {workspace.evidence.journalEntryCount}
              </strong>
            </div>
            <div className="rounded-xl border bg-white p-4">
              <span className="text-sm text-[var(--muted)]">Journal lines</span>
              <strong className="mt-2 block text-2xl">
                {workspace.evidence.journalLineCount}
              </strong>
            </div>
            <div className="rounded-xl border bg-white p-4">
          ...
          <section className="rounded-xl border bg-white p-5">
            <h2 className="flex items-center gap-2 text-xl font-semibold">
              <ShieldCheck className="size-5 text-[var(--brand)]" /> Readiness
              checks
            </h2>
            {blocked ? (
              <div className="mt-4 space-y-3">
                {workspace.evidence.blockers.map((blocker) => (
                  <div
                    key={blocker.code}
                    className="rounded-lg bg-amber-50 p-4 text-sm text-amber-950"
                  >
                    <strong>
                      {blocker.count} issue{blocker.count === 1 ? "" : "s"}
                    </strong>
                    <p>{blocker.message}</p>
                  </div>
                ))}
              ...
              <div className="mt-4 rounded-lg bg-emerald-50 p-4 text-sm text-emerald-900">
                <strong>All close checks passed.</strong>
                <p>
                  The ledger balances and no unresolved operational records
                  block preparation.
                </p>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-start justify-between gap-3 p-5">
              <div>
                ...
                <p className="text-sm text-[var(--muted)]">
                  Amounts are shown in naira with kobo retained to two decimal
                  places.
                </p>
              </div>
              <Button variant="secondary" onClick={downloadTrialBalance}>
              ...
              <table className="w-full min-w-[44rem] text-left text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="p-3 text-right">Net debit / (credit)</th>
                  </tr>
                </thead>
                <tbody>
                  {workspace.evidence.trialBalance.map((line) => (
                    <tr key={line.accountCode} className="border-t">
                      ...
                      <td className="p-3 text-right">
                        {formatNaira(line.debitMinor)}
                      </td>
                      <td className="p-3 text-right">
                        {formatNaira(line.creditMinor)}
                      </td>
                      <td className="p-3 text-right">
                        {formatNaira(line.netMinor)}
                      </td>
                    </tr>
                  ))}
                  {!workspace.evidence.trialBalance.length && (
                    <tr className="border-t">
                      <td
                        colSpan={4}
                        className="p-6 text-center text-[var(--muted)]"
                      >
                        No journals were posted in this month.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <section className="rounded-xl border bg-white p-5">
            <h2 className="flex items-center gap-2 text-xl font-semibold">
              <LockKeyhole className="size-5 text-[var(--brand)]" /> Close
              control
            </h2>
            <label className="mt-4 block text-sm">
              ...
              <textarea
                ...
                placeholder="Record review notes or references for the audit trail."
              />
            </label>
            <div className="mt-4 flex flex-wrap items-center gap-3">
              {!selectedPeriod || selectedPeriod.status === "open"
                ? can("accounting.close.prepare") && (
                    <Button
                      disabled={busy || blocked}
                      onClick={() =>
                        void run(
                          () =>
                            callAdministration("prepareAccountingPeriodClose", {
                              periodKey,
                              notes: notes || undefined,
                              idempotencyKey: crypto.randomUUID(),
                            }),
                          `${periodKey} prepared for completion.`,
                        )
                      }
                    >
                      <ShieldCheck className="mr-2 size-4" /> Prepare month
                    </Button>
                  )
                : null}
              {selectedPeriod?.status === "prepared" &&
                selectedPeriod.preparedBy === user?.uid &&
                !canApproveOwnWork && (
                  <span className="text-sm text-amber-800">
                    Another authorized finance officer or administrator must
                    complete this close.
                  </span>
                )}
              {selectedPeriod?.status === "prepared" &&
                (selectedPeriod.preparedBy !== user?.uid ||
                  canApproveOwnWork) &&
                can("accounting.close.approve") && (
                  <Button
                    disabled={busy || blocked}
                    onClick={() =>
                      void run(
                        () =>
                          callAdministration("completeAccountingPeriodClose", {
                            accountingPeriodId: selectedPeriod.id,
                            notes: notes || undefined,
                            idempotencyKey: crypto.randomUUID(),
                          }),
                        `${periodKey} closed and locked.`,
                      )
                    }
                  >
              ...
              {selectedPeriod?.status === "closed" && (
                <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-sm font-medium text-emerald-900">
                  <CheckCircle2 className="size-4" /> Month closed
                </span>
              )}
            </div>
          </section>
          ...
          <section className="rounded-xl border bg-white p-5">
            <h2 className="text-xl font-semibold">Close history</h2>
              ...
              {workspace.periods.map((period) => (
                ...
                >
                  <span>
                    ...
                    <small className="ml-2 capitalize text-[var(--muted)]">
                      {period.status}
                    </small>
                  </span>
                  <span className="text-sm">
                    {formatNaira(period.totalDebitMinor ?? 0)}
                  </span>
                </div>
              ))}
              {!workspace.periods.length && (
                <p className="text-sm text-[var(--muted)]">
                  No accounting month has been prepared yet.
                </p>
              )}
            </div>
          </section>
