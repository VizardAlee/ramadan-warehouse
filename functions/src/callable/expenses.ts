...
function requireExpenseScope(
  ...
  if (!branchId && !warehouseId && !organizationWide(actor))
    throw new HttpsError(
      "failed-precondition",
      "Select an assigned branch or warehouse for this expense.",
    );
}
function expenseVisible(
  actor: Awaited<ReturnType<typeof requireAccess>>,
  ...
  return Boolean(
    (branchId && actor.branchIds.includes(branchId)) ||
      (warehouseId && actor.warehouseIds.includes(warehouseId)),
  );
}
function writeJournal(
  ...
  const now = FieldValue.serverTimestamp(),
    journalNumber = numbered("JRN", values.sequence);
  transaction.set(values.counter, {
...
export const getExpenseWorkspace = onCall(
  { enforceAppCheck },
  async (request) => {
    const actor = await requireAccess(request);
    requirePermission(actor, "expenses.read");
    const input = parseInput(expenseWorkspaceInput, request.data);
    ...
    return {
      ...
      branches: branches.docs
        .filter(
          (document) =>
            organizationWide(actor) || actor.branchIds.includes(document.id),
        )
        .map((document) => ({
          id: document.id,
      ...
      warehouses: warehouses.docs
        .filter(
          (document) =>
            organizationWide(actor) || actor.warehouseIds.includes(document.id),
        )
        .map((document) => ({
          id: document.id,
          name: document.get("name"),
...
export const createExpense = onCall({ enforceAppCheck }, async (request) => {
  ...
  const category = db.doc(
    `expenseCategories/${uniquenessDocumentId(actor.organizationId, normalizedCategory)}`,
  );
  const categoryCounter = db.doc(
    `expenseCategoryCounters/${actor.organizationId}`,
  );
  const operation = db.doc(
    `idempotencyKeys/${actor.organizationId}_createExpense_${input.idempotencyKey}`,
  );
  const branch = input.branchId ? db.doc(`branches/${input.branchId}`) : null;
  const warehouse = input.warehouseId
    ? db.doc(`warehouses/${input.warehouseId}`)
    : null;
  const documentLock = input.supplierDocumentNumber
    ? db.doc(
        `expenseDocumentCodes/${uniquenessDocumentId(actor.organizationId, normalizeInventoryIdentifier(input.payeeName), normalizeInventoryIdentifier(input.supplierDocumentNumber))}`,
      )
    : null;
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (
      ...
      throw new HttpsError("failed-precondition", "Warehouse is unavailable.");
    if (documentSnapshot?.exists)
      throw new HttpsError(
        "already-exists",
        "This payee document number has already been recorded.",
      );
    const sequence = Number(counterSnapshot.get("value") ?? 0) + 1,
      expenseNumber = numbered("EXP", sequence),
      now = FieldValue.serverTimestamp();
    let categoryCode = String(categorySnapshot.get("code") ?? "");
    if (!categorySnapshot.exists) {
      const categorySequence =
        Number(categoryCounterSnapshot.get("value") ?? 0) + 1;
      categoryCode = `EXC-${String(categorySequence).padStart(4, "0")}`;
      transaction.set(categoryCounter, {
        organizationId: actor.organizationId,
    ...
    transaction.create(
      ...
      clean({
        ...
        categoryCode,
        categoryName: categorySnapshot.exists
    ...
    if (documentLock)
      transaction.create(documentLock, {
        organizationId: actor.organizationId,
...
async function expenseStatusAction(
  ...
  requirePermission(
    actor,
    action === "submit" ? "expenses.create" : "expenses.approve",
  );
  ...
  requireExpenseScope(
    actor,
    initial.get("branchId") || undefined,
    initial.get("warehouseId") || undefined,
  );
  const operation = db.doc(
    `idempotencyKeys/${actor.organizationId}_${action}Expense_${input.idempotencyKey}`,
  );
  ...
  const effectiveAt = Timestamp.fromDate(
    new Date(String(initial.get("expenseDate"))),
  );
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!current.exists || current.get("status") !== requiredStatus)
      throw new HttpsError(
        "failed-precondition",
        `Only a ${requiredStatus} expense can be ${action}ted.`,
      );
    if (
      action === "approve" &&
    ...
    if (action === "submit") {
      ...
      const journalCounterSnapshot = snapshots[2]!,
      ...
      writeJournal(transaction, actor, {
        ...
        sequence: Number(journalCounterSnapshot.get("value") ?? 0) + 1,
        journalType: "operating_expense",
        referenceType: "expense",
        referenceId: expense.id,
        referenceNumber: String(current.get("expenseNumber")),
    ...
    writeAuditLog(transaction, actor, {
      action: action === "approve" ? "expense.approved" : "expense.submitted",
      entityType: "expense",
      entityId: expense.id,
      correlationId: correlationId(),
      sourceFunction: `${action}Expense`,
      after: clean({
        status: action === "submit" ? "submitted" : "approved",
        journalEntryId: action === "approve" ? journal.id : undefined,
      }),
    });
  });
  return {
...
export const submitExpense = onCall({ enforceAppCheck }, (request) =>
  expenseStatusAction(request, "submit"),
);
export const approveExpense = onCall({ enforceAppCheck }, (request) =>
  expenseStatusAction(request, "approve"),
);

export const recordExpensePayment = onCall(
  { enforceAppCheck },
  async (request) => {
    const actor = await requireAccess(request);
    ...
    const operation = db.doc(
      `idempotencyKeys/${actor.organizationId}_recordExpensePayment_${input.idempotencyKey}`,
    );
    const payment = db.collection("expensePayments").doc(),
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (
        !current!.exists ||
        !["approved", "partially_paid"].includes(String(current!.get("status")))
      )
        throw new HttpsError(
          "failed-precondition",
          "Only an approved outstanding expense can be paid.",
        );
      const outstanding = Number(current!.get("outstandingAmountMinor") ?? 0);
      if (input.amountMinor > outstanding)
        throw new HttpsError(
          "failed-precondition",
          "Payment exceeds the expense outstanding balance.",
        );
      ...
      writeJournal(transaction, actor, {
        ...
        counter: journalCounter,
        sequence: Number(journalCounterSnapshot!.get("value") ?? 0) + 1,
        journalType: "expense_payment",
        referenceType: "expensePayment",
        referenceId: payment.id,
        referenceNumber: paymentNumber,
        description: `Payment for ${String(current!.get("expenseNumber"))}`,
        branchId: current!.get("branchId") || undefined,
        warehouseId: current!.get("warehouseId") || undefined,
        effectiveAt,
        lines,
      ...
      transaction.create(
        ...
        clean({
          organizationId: actor.organizationId,
          expenseId: expense.id,
          expenseNumber: current!.get("expenseNumber"),
          paymentNumber,
      ...
      writeAuditLog(transaction, actor, {
        ...
        entityId: expense.id,
        correlationId: correlationId(),
        sourceFunction: "recordExpensePayment",
        after: {
