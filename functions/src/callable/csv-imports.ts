...
export const previewCsvImport = onCall({ enforceAppCheck }, async (request) => {
  ...
  return preview(input.kind, input.csv, await contextFor(actor, input.kind));
});
  ...
  try {
    for (let index = 0; index < result.validRows.length; index++) {
      ...
      if (input.kind === "products") {
        ...
        const normalizedSku = normalizeInventoryIdentifier(row.sku!);
        await db.runTransaction(async (transaction) => {
          ...
          if (owner.exists) throw new HttpsError("already-exists", `SKU ${row.sku} already exists.`);
          const now = FieldValue.serverTimestamp();
          transaction.create(db.doc(`products/${productId}`), { organizationId: actor.organizationId, sku: row.sku, normalizedSku, name: row.name, unitOfMeasure: row.unitOfMeasure, trackingType: row.trackingType, categoryId: row.categoryId || null, active: true, hasLedgerActivity: false, currency: "NGN", createdAt: now, createdBy: actor.userId, updatedAt: now, updatedBy: actor.userId });
          transaction.create(lock, { organizationId: actor.organizationId, normalizedSku, productId, updatedAt: now });
          transaction.set(db.doc(`productCosts/${productId}`), { organizationId: actor.organizationId, productId, defaultUnitCostMinor: Number(row.defaultUnitCostMinor), currency: "NGN", updatedAt: now, updatedBy: actor.userId });
          writeAuditLog(transaction, actor, { action: "product.imported", entityType: "product", entityId: productId, correlationId: correlationId(), sourceFunction: "confirmCsvImport", after: { sku: row.sku } });
        });
      } else {
