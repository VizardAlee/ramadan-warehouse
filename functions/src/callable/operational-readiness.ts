...
import { validateSimpleTransfer } from "../transfers/validate-simple-transfer.js";

const reconciliationInput = z.object({
  transferId: z.string().trim().min(1).max(180),
});
...
const canReconcile = (actor: AccessProfile) =>
  (
    ["system_administrator", "operations_administrator", "auditor"] as const
  ).some((roleId) => hasRole(actor, roleId));

export const reconcileTransfer = onCall(
  { enforceAppCheck },
  async (request) => {
    const started = Date.now();
    const actor = await requireAccess(request);
    if (!canReconcile(actor))
      throw new HttpsError(
        "permission-denied",
        "Detailed reconciliation is restricted to administrators and auditors.",
      );
    const input = parseInput(reconciliationInput, request.data);
    await enforceRateLimit({
      organizationId: actor.organizationId,
      userId: actor.userId,
      operation: "transfer-reconciliation",
      limit: 30,
      windowSeconds: 60,
    });
    const cid = correlationId();
    const simple = await db.doc(`stockTransfers/${input.transferId}`).get();
    const result = simple.exists
      ? await validateSimpleTransfer(actor.organizationId, input.transferId)
      : await validateTransferInvariants(
          actor.organizationId,
          input.transferId,
        );
    logOperation({
      correlationId: cid,
      functionName: "reconcileTransfer",
      organizationId: actor.organizationId,
...
export const reconcileWarehouseOperations = onCall(
  { enforceAppCheck },
  async (request) => {
    const actor = await requireAccess(request);
    if (!canReconcile(actor))
      throw new HttpsError(
        "permission-denied",
        "Operational reconciliation is restricted to administrators and auditors.",
      );
    const input = parseInput(operationsInput, request.data);
    await enforceRateLimit({
      organizationId: actor.organizationId,
      userId: actor.userId,
      operation: "operations-reconciliation",
      limit: 10,
      windowSeconds: 60,
    });
      ...
      ? new Set(
          (
            await db
              ...
              .limit(500)
              .get()
          ).docs.map((item) => String(item.get("transferId"))),
        )
      : undefined;
    ...
    const filtered = transferDocs.filter((item) => {
      ...
      return (
        ...
        (!input.branchId ||
          item.get("destinationBranchId") === input.branchId) &&
        (!input.warehouseId ||
          item.get("originWarehouseId") === input.warehouseId) &&
        (!input.requestId || item.get("sourceRequestId") === input.requestId) &&
        (!productTransferIds || productTransferIds.has(item.id)) &&
        (!input.startAt ||
          (createdAt !== undefined &&
            createdAt.getTime() >= new Date(input.startAt).getTime())) &&
        (!input.endAt ||
          (createdAt !== undefined &&
            createdAt.getTime() <= new Date(input.endAt).getTime()))
      );
    });
    const results = await Promise.all(
      filtered.map((item) =>
        validateTransferInvariants(actor.organizationId, item.id),
      ),
    );
    const simpleDocs = input.transferId
      ? [await db.doc(`stockTransfers/${input.transferId}`).get()]
      : (
          await db
    ...
    const simpleFiltered = simpleDocs
      .filter(
        (d) =>
          d.exists &&
          d.get("organizationId") === actor.organizationId &&
          (!input.branchId ||
            d.get("destinationBranchId") === input.branchId ||
            d.get("sourceBranchId") === input.branchId) &&
          (!input.warehouseId ||
            d.get("sourceWarehouseId") === input.warehouseId) &&
          (!input.requestId || d.get("sourceRequestId") === input.requestId) &&
          (!input.productId ||
            (d.get("items") as Array<{ productId: string }>).some(
              (i) => i.productId === input.productId,
            )) &&
          (!input.startAt || d.get("createdAt") >= input.startAt) &&
          (!input.endAt || d.get("createdAt") <= input.endAt),
      )
      .sort((a, b) =>
    ...
    results.push(
      ...(await Promise.all(
        simpleFiltered.map((d) =>
          validateSimpleTransfer(actor.organizationId, d.id),
        ),
      )),
    );
    const requestChecks = await reconcileOrganizationRequests(
      actor.organizationId,
    );
    return {
      organizationId: actor.organizationId,
      ...
      summary: {
        ...
        requestFulfilmentFailures: requestChecks.filter(
...
export const systemReadiness = onCall({ enforceAppCheck }, async (request) => {
  const actor = await requireAccess(request);
  if (
    !(["system_administrator", "operations_administrator"] as const).some(
      (roleId) => hasRole(actor, roleId),
    )
  )
    throw new HttpsError(
      "permission-denied",
      "Detailed readiness is administrator-only.",
    );
  ...
  return {
    status: ["goods_in_transit", "damaged"].every((kind) => kinds.has(kind))
      ? "ready"
      : "not_ready",
    checkedAt: new Date().toISOString(),
    checks: {
      ...
      requiredVirtualLocations: Object.fromEntries(
        ["goods_in_transit", "damaged", "quarantined", "returned"].map(
          (kind) => [kind, kinds.has(kind)],
        ),
      ),
    },
  };
