...
const code = z.string().trim().min(2).max(24).regex(/^[A-Z0-9_-]+$/);
const text = (max: number) => z.string().trim().max(max).optional();
export const organizationInput = z.object({ legalName: z.string().trim().min(2).max(160), tradingName: text(160), code, registrationNumber: text(80), contactEmail: z.string().trim().email().optional(), phoneNumbers: z.array(z.string().trim().min(7).max(24)).max(5).default([]), address: text(500), defaultCurrency: z.literal("NGN").default("NGN"), timezone: z.string().trim().min(1).default("Africa/Lagos") });
export const bootstrapInput = z.object({ organization: organizationInput, bootstrapSecret: z.string().min(16).max(512).optional() });
export const userInput = z.object({ email: z.string().trim().toLowerCase().email().max(254), displayName: z.string().trim().min(2).max(120), phoneNumber: z.string().trim().min(7).max(24).optional(), employeeReference: text(80), roleId: z.enum(roles), branchIds: z.array(id).max(100).default([]), warehouseIds: z.array(id).max(100).default([]), status: z.enum(["active", "inactive", "suspended"]).default("active"), idempotencyKey: z.string().uuid() });
...
export const branchInput = baseMaster.extend({ contactEmail: z.string().trim().email().optional(), contactPhone: text(24), managerUserId: id.optional(), idempotencyKey: z.string().uuid() });
